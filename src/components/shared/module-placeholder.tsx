      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
        <Card className="min-h-[460px] p-6 sm:p-9">
          <div className="flex flex-wrap items-center gap-2">
            <Badge>{module.stage === "mvp" ? "Hackathon MVP" : "Future module"}</Badge>
            <Badge variant={isNext ? "success" : "demo"}>{isNext ? "Up next" : `Milestone ${module.milestone}`}</Badge>
          </div>
          <div className="mt-16 max-w-2xl">
            <div className="mb-6 grid size-14 place-items-center rounded-xl border border-primary/20 bg-primary/10 text-[#a998ff]"><Blocks className="size-6" /></div>
          ...
          <div className="mt-12 flex flex-wrap gap-3">
            {isNext ? (
              <Button asChild><Link href="/onboarding">Start with onboarding <ArrowRight /></Link></Button>
            ) : (
        ...
        <Card className="h-fit p-5">
          ...
          <div className="mt-5 grid gap-4 text-sm">
            {["Route and app shell", "Typed module registry", "Service interface"].map((item) => (
              <div className="flex items-center gap-3" key={item}><CheckCircle2 className="size-4 text-success" /><span>{item}</span></div>
            ))}
            <div className="flex items-center gap-3 text-muted-foreground"><CircleDashed className="size-4" /><span>Interactive workflow · M{module.milestone}</span></div>
          </div>
        </Card>
      </div>
    </div>
