  ...
  return (
    <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }} className={cn("rounded-xl border bg-card/78 p-5 shadow-[var(--shadow-card)]", selected ? "border-success/35" : "border-border", layout === "list" && "lg:grid lg:grid-cols-[auto_1fr_auto] lg:items-center lg:gap-5")}>
      ...
      <div className={cn("min-w-0", layout === "grid" ? "mt-4" : "mt-4 lg:mt-0")}>
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs"><span><strong className="text-foreground">{formatCurrency(college.estimatedAnnualCost)}</strong> / year</span><span><strong className="text-foreground">{college.placementRateDemo}%</strong> placement demo</span><span><strong className="text-foreground">{college.medianPackageDemo}</strong></span></div>
      </div>
